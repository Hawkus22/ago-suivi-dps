...
**Suivi départemental des DPS et coordination des renforts secouristes — Côtes-d'Armor (22)**
...
**PHARE** automatise ce suivi semaine par semaine, pour les 11 antennes du département.
...
```
PC / Mac
└── PHARE-v1.1.0.exe        (PyQt6 + Python, onefile PyInstaller)
...
| `pictures/` | Logos et visuels (PHARE Logo, bannière, Hawkus Corp) |
...
```
PHARE-v1.1.0.exe    (Windows)
PHARE-v1.1.0        (macOS Apple Silicon)
ago_config.ini      (à joindre manuellement — contient le mot de passe Supabase)
```
